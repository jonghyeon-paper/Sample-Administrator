	...
	<article id="mycontent" style="min-height: 649px;">
		...
		<div>
			<div class="contbox">
				...
				<div class="ibas_graph_wrap" style="margin-top: 30px;" id="buttonArea">
		...
		var drawUserListObject= function(data){
			...
			for (let item of userList) {
				...
				$('<td>').addClass('txt1')
				         ...
				         .appendTo($tr);
				$('<td>').addClass('txt1')
				         .append($('<a>').attr({href: '#', onclick: 'return false;', id: 'edit-' + item['userId']})
				                         .addClass('hide_btn')
				                         .html('Edit')
				                )
				         .append($('<span>').css({padding: '0px 10px;'}))
				         .appendTo($tr);
			}
			
			$table.on('click', 'a[id^=edit-]', function() {
				var userInfo = $(this).parentsUntil('tr').parent().data('userInfo');
				
				$('<form>').attr({action: '${contextPath}/user/writeview.do', method: 'get'})
